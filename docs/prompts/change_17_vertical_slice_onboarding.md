...
Quiero iniciar un nuevo change SDD/OpenSpec para migrar el módulo `onboarding` a arquitectura vertical slice.
Change: `17-vertical-slice-onboarding`
Objetivo:
Refactorizar únicamente `internal/modules/onboarding/` hacia vertical slice, manteniendo el módulo como frontera principal y haciéndolo más autocontenido. No migrar otros módulos en este change.
