...
Quiero iniciar un nuevo change SDD/OpenSpec para migrar el módulo `auth` a arquitectura vertical slice.
Change: `16-vertical-slice-auth`
Objetivo:
Refactorizar únicamente `internal/modules/auth/` hacia vertical slice, manteniendo el módulo como frontera principal y haciéndolo más autocontenido. No migrar otros módulos en este change.
