...
Quiero iniciar un nuevo change SDD/OpenSpec para migrar el módulo `company` a arquitectura vertical slice.
Change: `15-vertical-slice-company`
Objetivo:
Refactorizar únicamente `internal/modules/company/` hacia vertical slice, manteniendo el módulo como frontera principal y haciéndolo más autocontenido. No migrar otros módulos en este change.
