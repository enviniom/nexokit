...
Replace `internal/modules/companies/{handler,service,repository}.go` with one package per existing public endpoint. Shared non-query concerns move to `internal/modules/companies/core`; reusable DB query logic moves to `internal/modules/companies/queries` with one test file per query file. There is no `create_company` slice because there is no public `POST /api/v1/companies`; company creation remains owned by onboarding. The company detail endpoint becomes `view_company`, matching `companies:view` permission semantics already used by `routes.go` for `GET /companies/:id`.
...
| Shared types | `companies/core` holds models, DTOs/contracts, enums/constants, errors, and shared values that are not query logic | Keep shared files in root `companies`; duplicate per slice | Avoids Go import cycles while preserving the companies module boundary. |
...
| Repositories | Narrow per-slice repos over shared `*gorm.DB` and `queries` helpers; shared contracts stay in `companies/core` | God repository; duplicated models | Keeps slice ownership clear while deduplicating repeated query logic. |
...
|---|---|---|
| `internal/modules/companies/core/model.go` | Create | Move `Company`, `CompanyDomain`, constants, and shared model contracts here. |
| `internal/modules/companies/core/dto.go` | Create | Move shared request/response DTOs, validators, normalization, mappers, shared errors here. |
| `internal/modules/companies/core/error.go` | Create | Shared companies errors (`duplicate domain`, `active primary exists`, `domain ownership`). |
| `internal/modules/companies/core/contracts.go` | Create | Shared interfaces/contracts needed by slices or middleware. |
...
| `internal/modules/companies/{model,dto,handler,service,repository}.go` | Delete/Move | Shared model/DTO contents move to `core`; flat layers removed after route/test parity. |
...
`companies.Container` exposes handlers by slice name and a resolver implementing `middleware.CompanyResolver`. Slices MUST import shared types/contracts from `internal/modules/companies/core`, never root `companies`, and MUST NOT import sibling slices. Repositories MAY import `internal/modules/companies/queries` for duplicated query methods. Root container MUST NOT import `internal/modules/companies/*` subpackages except the root `companies` package.
...
1. Add module `container.go`, `core/` contracts/models/DTOs, resolver, and seven slice packages while old flat files still compile.
