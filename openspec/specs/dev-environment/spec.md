...
The system MUST provide Makefile targets: `build`, `test`, `run`, `migrate-up`, `migrate-down`, `migrate-create`, `migrate-status`, `fmt`, `vet`, `install-hooks`, `uninstall-hooks`, `check-env`. All MUST appear in `.PHONY`.
