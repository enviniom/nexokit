...
First-class `company_domains` model for multi-domain tenant ownership. Replaces single-host company ownership (`companies.domain` and `companies.subdomain`) with a one-to-many table supporting multiple hostnames per company, exact host resolution, optional technical platform domains, and redirect-capable aliases.
