...
func (f *fakeCache) Get(context.Context, string) ([]byte, error) { return f.values["rbac:permissions:u1"], nil }
func (f *fakeCache) Set(_ context.Context, key string, value []byte, d time.Duration) error {
	if f.sets == nil { f.sets = map[string][]byte{}; f.ttl = map[string]time.Duration{} }
	f.sets[key] = value; f.ttl[key] = d; return nil
}
