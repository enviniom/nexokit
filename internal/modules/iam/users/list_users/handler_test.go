...
func (f fakeHandlerService) List(_ tenant.TenantContext, _ query.ListParams) ([]core.UserResponse, int64, error) {
	if f.err != nil {
		return nil, 0, f.err
	}
...
func TestHandlerHandle(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			service:    fakeHandlerService{items: []core.UserResponse{{PublicID: "user-1", Name: "Alice", Email: "alice@example.com"}}, total: 1},
			...
			assertBody: func(t *testing.T, payload map[string]any) {
				...
				if !ok || len(data) != 1 {
					t.Fatalf("expected one user in data")
				}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			c.Request = httptest.NewRequest(http.MethodGet, "/iam/users?page=1&per_page=20", nil)
