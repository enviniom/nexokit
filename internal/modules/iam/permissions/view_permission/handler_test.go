...
func (f fakeHandlerService) GetByPublicID(string) (*core.PermissionResponse, error) {
	if f.err != nil {
		return nil, f.err
	}
	return f.item, nil
...
func TestHandlerHandle(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "returns success payload", service: fakeHandlerService{item: &core.PermissionResponse{PublicID: "perm-1"}}, statusCode: http.StatusOK},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			c.Request = httptest.NewRequest(http.MethodGet, "/iam/permissions/perm-1", nil)
