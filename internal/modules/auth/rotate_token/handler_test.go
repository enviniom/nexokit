...
type fakeService struct {
	result *core.TokenPairResponse
	err    error
}

func (f fakeService) Rotate(req core.RefreshRequest) (*core.TokenPairResponse, error) {
	if f.err != nil {
		return nil, f.err
	}
	return f.result, nil
...
func TestHandler_Handle(t *testing.T) {
	...
	t.Run("returns rotated token pair", func(t *testing.T) {
		h := NewHandler(fakeService{result: &core.TokenPairResponse{AccessToken: "new-access", RefreshToken: "new-refresh"}})
		w := httptest.NewRecorder()
		c, _ := gin.CreateTestContext(w)
		c.Request = jsonRequest(http.MethodPost, "/auth/refresh", core.RefreshRequest{RefreshToken: "old-refresh"})
	...
	t.Run("returns validation error", func(t *testing.T) {
		...
		w := httptest.NewRecorder()
		c, _ := gin.CreateTestContext(w)
		c.Request = jsonRequest(http.MethodPost, "/auth/refresh", core.RefreshRequest{})
