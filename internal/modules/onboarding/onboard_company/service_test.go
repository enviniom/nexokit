...
func (f fakePasswordHasher) HashPassword(password string) (string, error) { return "hashed_" + password, nil }
...
func newService(db *gorm.DB, platformDomain string) Service {
	return NewService(db, NewRepository(), fakePasswordHasher{}, platformDomain)
}
...
func TestService_Onboard_Success(t *testing.T) {
	...
	if res.CompanySlug != "acme" || res.AdminEmail != "jane@acme.com" { t.Fatalf("unexpected response: %#v", res) }
}

func TestService_Onboard_CreatesPrimaryDomain(t *testing.T) {
	db := setupTestDB(t); seedPermissions(t, db); svc := newService(db, "")
	domain := " DreamMakers.COM.CO "
	req := baseRequest(); req.Slug = "dreammakers"; req.AdminEmail = "jane@dreammakers.com.co"; req.Domain = &domain
	if _, err := svc.Onboard(context.Background(), req); err != nil { t.Fatalf("onboard: %v", err) }
	var d core.OnboardingCompanyDomain
	if err := db.Where("domain = ?", "dreammakers.com.co").First(&d).Error; err != nil { t.Fatalf("expected primary domain: %v", err) }
	if d.Kind != core.DomainKindPrimary { t.Fatalf("unexpected kind: %s", d.Kind) }
}

func TestService_Onboard_CreatesTechnicalDomainWhenRequested(t *testing.T) {
	db := setupTestDB(t); seedPermissions(t, db); svc := newService(db, "KilaShop.COM")
	req := baseRequest(); req.Slug = " DreamMakers "; req.AdminEmail = "jane@dreammakers.com.co"; req.GenerateTechnicalDomain = true
	if _, err := svc.Onboard(context.Background(), req); err != nil { t.Fatalf("onboard: %v", err) }
	var d core.OnboardingCompanyDomain
	if err := db.Where("domain = ?", "dreammakers.kilashop.com").First(&d).Error; err != nil { t.Fatalf("expected technical domain: %v", err) }
}

func TestService_Onboard_SkipsTechnicalDomainWhenNotRequested(t *testing.T) {
	db := setupTestDB(t); seedPermissions(t, db); svc := newService(db, "kilashop.com")
	req := baseRequest(); req.Slug = "dreammakers"; req.AdminEmail = "jane@dreammakers.com.co"
	if _, err := svc.Onboard(context.Background(), req); err != nil { t.Fatalf("onboard: %v", err) }
	var count int64
	db.Model(&core.OnboardingCompanyDomain{}).Where("domain = ?", "dreammakers.kilashop.com").Count(&count)
	if count != 0 { t.Fatalf("expected technical domain skipped, got %d", count) }
}
...
func TestService_Onboard_DuplicateDomain_Rollback(t *testing.T) {
	...
	domain := "dreammakers.com.co"; req := baseRequest(); req.Slug = "dreammakers"; req.AdminEmail = "jane@dreammakers.com.co"; req.Domain = &domain
	_, err := svc.Onboard(context.Background(), req)
	if !errors.Is(err, core.ErrDuplicateCompanyDomain) { t.Fatalf("expected duplicate domain, got: %v", err) }
}
...
func TestService_Onboard_DuplicateTechnicalDomain_Rollback(t *testing.T) {
	...
	req := baseRequest(); req.Slug = "dreammakers"; req.AdminEmail = "jane@dreammakers.com.co"; req.GenerateTechnicalDomain = true
	_, err := svc.Onboard(context.Background(), req)
	if !errors.Is(err, core.ErrDuplicateTechnicalDomain) { t.Fatalf("expected duplicate technical domain, got: %v", err) }
}

func TestService_Onboard_PrimaryDomainCannotEqualGeneratedTechnicalDomain(t *testing.T) {
	db := setupTestDB(t); seedPermissions(t, db); svc := newService(db, "kilashop.com")
	domain := "dreammakers.kilashop.com"; req := baseRequest(); req.Slug = "dreammakers"; req.AdminEmail = "jane@dreammakers.com.co"; req.Domain = &domain; req.GenerateTechnicalDomain = true
	_, err := svc.Onboard(context.Background(), req)
	if !errors.Is(err, core.ErrDuplicateTechnicalDomain) { t.Fatalf("expected duplicate technical domain, got: %v", err) }
}
...
func TestService_Onboard_DuplicateSlug_Rollback(t *testing.T) {
	...
	if !errors.Is(err, core.ErrDuplicateCompanySlug) { t.Fatalf("expected duplicate slug, got: %v", err) }
}
...
func TestService_Onboard_DuplicateEmail_Rollback(t *testing.T) {
	...
	req := baseRequest(); req.Slug = "acme-new"
	_, err := svc.Onboard(context.Background(), req)
	if !errors.Is(err, core.ErrDuplicateAdminEmail) { t.Fatalf("expected duplicate email, got: %v", err) }
}
